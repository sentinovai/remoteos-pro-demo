(function(){
  ...
  const STORAGE_KEY = 'remoteos-pro-v5-final';
  ...
  const DEFAULT_ATTACHMENT_PAYLOADS = {
    'banner-pack.zip': { mime:'application/zip', content:btoa('RemoteOS Pro bundled delivery archive\nVersion: v1.0\nContains: exported banners, source references, and review checklist.') },
  ...
  function actionButton(config){
    ...
    return `<button class="${config.secondary ? 'ghost-btn' : 'primary-btn'}" ${attrs.join(' ')}>${esc(config.label || 'Open')}</button>`;
  }
  ...
  function exportJson(){
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = 'remoteos-pro-workspace.json';
    a.click();
  ...
  function render(){
    syncSelectedEntities();
    if(location.hash.replace('#','') !== state.currentView && window.history?.replaceState){
      window.history.replaceState(null, '', `${location.pathname}${location.search}#${state.currentView}`);
    }
